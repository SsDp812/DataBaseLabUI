...
public class ViewReportsWindowForAdmin extends StandartWindow {
    private JTable reportTable;
    String[] colNames;
    String[][] info;
    public ViewReportsWindowForAdmin(String title, JFrame back) throws HeadlessException, SQLException {
        super(title);
        this.backFrame = back;
        initBackButton(backFrame,this);
        colNames = new String[]{"Supply price","Date","Producer city","Producer name"};
        Connection connection = ConnectionMaker.getConnect();
        PreparedStatement statement = connection.prepareStatement("SELECT * FROM supplies_view");
